...
/**
 * Selects an unlock flow according to the currently set unock type.
 * The unlock type is pulled from <application_native_id>.json file,"GD_TEST_UNLOCK_TYPE" field.
 * In case current activation type is "idp", then "Sign in" option is selected,
 * Otherwise, the default provisioning flow will be triggered by selecting "Enter credentials" option.
 *
 *  @return YES, if unlock type was selected, otherwise NO
 */
- (BOOL)selectRemoteUnlockType;

/**
 * Selects a forgot password flow according to the currently set unock type.
 * The unlock type is pulled from <application_native_id>.json file,"GD_TEST_UNLOCK_TYPE" field.
 * In case current activation type is "idp", then "Sign in" option is selected,
 * Otherwise, the default provisioning flow will be triggered by selecting "Enter credentials" option.
 *
